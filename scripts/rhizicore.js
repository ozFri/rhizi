...
var boxedin,nodetext,linktext,link,links,node,circle;
...
function myGraph(el) {
    ...
    this.addNode = function (id,type,state) {
      ...
      if(node !== undefined) {
        ...
        nodes.push({"id":id,"type":type,"state":state,"start":start,"end":end});
        update();
      }
    }
...
var findNode = function (id,state) {
  ...
  .text(function(d) { if(d.name.length<25)return d.name; else return d.name.substring(0,14)+"..."; })
      ...
      force.on("tick", tick);
        ...
        for (var i = 0; i < nodes.length; i++) {
          ...
          if(current.type==="deliverable"){
           deliverables.push({"id":nodes[i].id,"startdate":nodes[i].start,"enddate":nodes[i].end});
         }
          //Do something
        }
